...
export default function Home() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      form.reset();
      setNote({
        text: "Thanks — your message is with our team. We'll reply within one business day.",
        color: "var(--teal)",
      });
    } catch (err) {
      console.error("Contact form error:", err);
      // Fall back to the visitor's mail client so a failed request never
      // costs the lead outright.
      ...
      setNote({
        text: "We couldn't send that automatically — your email app should now open instead.",
        color: "var(--accent)",
      });
    } finally {
      setSending(false);
